...
urllib.request.urlretrieve("https://www.post.japanpost.jp/zipcode/dl/kogaki/zip/ken_all.zip", "ken_all.zip")

with ZipFile("ken_all.zip") as zip_ref:
	with zip_ref.open("KEN_ALL.CSV") as file:
		lines = file.read().decode("SJIS")

lines = unicodedata.normalize('NFKC', lines)
...
for i in range(len(lines)):
	...
	str_ng = ["○", "〔", "〜", "、", "「", "を除く", "以外", "その他", "地割", "不明", "以下に掲載がない場合"]
